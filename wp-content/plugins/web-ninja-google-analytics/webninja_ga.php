...
if ( !function_exists('sys_get_temp_dir')) 
{
  function sys_get_temp_dir() 
  {
    ...
    if (!empty($upload_dir['path'])) { return $upload_dir['path']; }
...
class SimpleFileCache
{
  function canCache()
  {
    ...
    $filename = sys_get_temp_dir() . '/wnga_cache_' . md5('cachetest') . '.dat';
  ...
  {
    $filename = sys_get_temp_dir() . '/wnga_cache_' . md5($key) . '.dat';
  ...
  {
    $lh = SimpleFileCache::lock();
    foreach(glob(sys_get_temp_dir() . '/wnga_cache_*.dat') as $filename) 
    {
      unlink($filename);
    }
    SimpleFileCache::unlock($lh);
  ...
  function lock()
  {
    $filename = sys_get_temp_dir() . '/wnga_lock.dat';
    if(file_exists($filename))
    {
...
if (version_compare($wp_version, '2.8', '>=')) 
{
  ...
  class WNGAWidget extends WP_Widget 
  {
    function WNGAWidget() 
    {
...
if (version_compare($wp_version, '2.8', '>=')) 
{
  //add_action('widgets_init', create_function('', 'return register_widget("WNGAWidget");'));
}
...
function wbga_get_option($option_name) {
	...
	if (!$wbga_options || !array_key_exists($option_name, $wbga_options)) {
		...
		$wbga_default_options['account_id']             = 'UA-10605157-8';  
		...
		$wbga_default_options['tracking_type']        	= 1; 
...
function wbga_options() {
  ...
  if (isset($_POST['info_update'])) {
	...
	$wbga_options['tracking_type']        	= $_POST['tracking_type'];
...
<br />
<center><form class="gcheckout" method="POST" action="https://checkout.google.com/cws/v2/Merchant/462781349183533/checkoutForm" accept-charset="utf-8"> <input name="item_name_1" value="Web Ninja Google Analytics" type="hidden"> <input name="item_description_1" value="Thanks! Every little bit helps!" type="hidden"> <input name="item_quantity_1" value="1" id="qty" type="hidden"> <label><b>Amount:</b> $</label><input name="item_price_1" value="" id="amt" type="text" size="10"> <input name="charset" type="hidden"> <br /><input id="submit" name="Google Checkout" alt="Fast checkout through Google" src="http://josh-fowler.com/images/donateNow.png" type="image"> </form></center><br />
<br />
    ...
    <fieldset class="options" name="tracking">
      ...
      <table width="500px" cellspacing="5" cellpadding="5" class="editform">
        ...
        <tr>
          ...
          <th nowrap valign="top">Tracking Type: <span onmouseover="tooltip.show('What are you tracking? A single domain (default), One domain with multiple subdomains, Multiple top-level domains', 400);" onmouseout="tooltip.hide();" style="color:#00F; cursor:pointer">[?]</span></th>
          <td><?php $tracking_type = wbga_get_option('tracking_type'); ?>
          <select name="tracking_type" id="tracking_type">
              <option value="1" <?php echo ($tracking_type == '1') ? 'selected' : ''; ?>>Single Domain</option>
              <option value="2" <?php echo ($tracking_type == '2') ? 'selected' : ''; ?>>Subdomains</option>
              <option value="3" <?php echo ($tracking_type == '3') ? 'selected' : ''; ?>>Multiple Top-level</option>
            </select></td>
		</tr>
        <tr>
...
?></td>
        ...
        <tr>
          ...
          <select name="ga_post_days" id="ga_display_level">
              <option value="0" <?php echo ($ga_post_days == '0') ? 'selected' : ''; ?>>Disabled</option>
...
function wbga_preg_callback($match) {
  ...
  if ($tracker) {
    ...
    $onClick="javascript:_gaq.push(['_trackPageview', '$tracker']);";
...
";
		} elseif (wbga_get_option('tracking_type') == 2) {
		  $result="
<!-- Google Analytics Code added by Web Ninja Google Analytics plugin v".wbgaversion.": http://josh-fowler.com/?page_id=70 -->
<script type='text/javascript'>
  var _gaq = _gaq || [];
  _gaq.push(['_setAccount', '".wbga_get_option('account_id')."']);
  _gaq.push(['_setDomainName', '.".wbga_get_domain(home_url())."']);
  _gaq.push(['_trackPageview']);
...
if (wbga_get_option('gad_post_days') > 0) { 
...
?>
    ...
    <table style="padding:0">
      <tr>
        <td style="border:0">
          <img width="90" height="30" src="http://chart.apis.google.com/chart?chs=90x30&cht=ls&chf=bg,s,FFFFFF00&chco=0077CC&chd=t:0&chds=0,0"/><br />
          0 pageviews<br/>
          0 exits<br/>
          0 uniques<br/>
        </td>
...
function dashboard_gad()
{
  ...
  $start_date_ts = time() - (60 * 60 * 24 * $dashdays); 
